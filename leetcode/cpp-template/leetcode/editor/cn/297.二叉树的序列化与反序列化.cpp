...
//     TreeNode* deserialize(std::list<std::string>& nodes) {
...
//         std::string str = nodes.front();
//         // 这里弹出头部不要用std::vector，他还要一步一步的挪，用std::list
//         nodes.pop_front();  // 这一步弹出的操作很关键，为了保证子问题拿到的数据和原问题自洽
//         if ("#" == str)
//             return nullptr;

//         TreeNode* root = new TreeNode(std::atoi(str.c_str()));
//         // 递归处理左右子树
