...
class LRUCache {
public:
    // 存储的数据我们用一个键值对std::pair<int,int>即可
    // 关键是我们怎么优化get通过键值查找数据的时间，首先我们肯定能想到哈希表，但是我们需要判断缓存队列的弹出，需要在小的时间复杂度内删除指定的值，或者把某个元素移动到头部，这么看起来双向链表的结构非常合适
    // 因此总结得出，关于get接口，我们通过unorder_map处理，键是int，值是链表的迭代器；然后我们维护一个std::list，根据键定位到迭代器，然后就可以操作std::list了
    ...
    void put(int key, int value) {
        ...
        // 不存在就是插入值，需要判断是否超了，清理缓存
            ...
            um.erase(l.back().first);  // 就是这里需要l存储key所以才需要std::pair
            l.pop_back();              // 从l中删除
        }
...
private:
    ...
    std::unordered_map<int, std::list<std::pair<int, int>>::iterator> um;
